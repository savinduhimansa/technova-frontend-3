...
import { FaTruckField, FaList, FaCircleXmark, FaFloppyDisk, FaCircleCheck, FaPlus } from "react-icons/fa6";
import { SUPPLIERS } from "../api/api.js";

export default function AddSupplier() {
  // use backend field names: supplierName, contactNo
  ...
  const save = async (e) => {
    ...
    // payload that matches the model
    const payload = {
    ...
    try {
      ...
      if (err?.response?.status === 409) return showError(msg || "Duplicate supplierId or supplierName.");
      showError(msg);
    }
  };
      ...
      <main className="max-w-6xl mx-auto px-5 py-8">
        ...
        >
          <h2 className="text-xl font-bold mb-1" style={{ color: blue.textPri }}>
            <FaPlus className="mr-2 inline-block" style={{ color: blue.accent }} />
            New Supplier
          </h2>
          <p className="text-sm mb-6" style={{ color: blue.textSec }}>
            Fill details and save to MongoDB.
          </p>
